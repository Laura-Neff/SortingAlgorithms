...
//			a[i] = storeMin;
  ...
  private static void merge(long[] a, long[] aux, int lo, int mid, int hi) {
	...
	public static void MergeSortNonRec(long[] a) {
		...
		int total_size = a.length; //the size of the whole array
		int array_size = 1; //the number of elements per array

		long[] aux = new long[total_size];

		while (array_size < total_size) { //merge up from single arrays until we have the whole array
			for (int array_pointer = 0; array_pointer < total_size; array_pointer += 2*array_size) {
				int lo = array_pointer;
				int mid = array_pointer + array_size-1; //0-indexed
				int hi = array_pointer + 2 * array_size-1; //0-indexed
				merge(a, aux, lo, mid, hi);
			}
			array_size *= 2; //on each iteration, increase size by 2
		} //done when we have the whole array
